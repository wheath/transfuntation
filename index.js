...
//for the marker.
function addMarkerToMap(lat, long, htmlMarkupForInfoWindow, img_url, markerCount){
    console.log("_dbg in addMarkerToMap");
    var infowindow = new google.maps.InfoWindow();
...
window.addEventListener('message', function(event) {
    ...
    if (~event.origin.indexOf('http://yoursite.com')) {
        ...
        console.log('hello testFunc in frame');
    } else {
        // The data hasn't been sent from your site!
        // Be careful! Do not use it.
    console.log('hello testFunc in frame');
        img_url1 = 'http://www.graysonconsulting.biz/themes/bartik/images/dabus.jpg';
        img_url2 = 'http://www.graysonconsulting.biz/themes/bartik/images/mybus.png';
        bus_marker = window.addMarkerToMap(21.3348, -157.8875, 'Bus Marker', img_url1, 2000);
        my_bus_marker = window.addMarkerToMap(21.3548, -157.8875, 'My Bus Marker', img_url2, 2001);

        
        move = function(marker, stopIds, stopMarkersSelected, wait, index) {
        if(index >= stopIds.length-1) {
          if(wait == 1000) {
            console.log("_dbg calling window.top.postMessage");
            parent.postMessage('hello', '*');
          }
          return;
        }
        console.log("_dbg in move on index: " + index);
        console.log("_dbg stopIds[index]: " + stopIds[index]);
        var myLatLng = stopMarkersSelected[stopIds[index]].getPosition();
          
        marker.setPosition(new google.maps.LatLng(myLatLng.lat()+ .01, myLatLng.lng() - .01));
        if(index < stopIds.length) {
          // call the next "frame" of the animation
          setTimeout(function() { 
            move(marker, stopIds, stopMarkersSelected, wait, index+1);
          }, wait);
        }
        }
        ...
        for (var stopId in stopMarkersSelected) {
          ...
          console.log("_dbg stopId: " + stopId);
          stopIds.push(stopId);
        }
        move(bus_marker, stopIds, stopMarkersSelected, 2000, 0); 
        move(my_bus_marker, stopIds, stopMarkersSelected, 1000, 0); 
        return;
    }
});
